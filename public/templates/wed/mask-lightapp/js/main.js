...
//定义这个全局变量用于跨站点图片
host_base_url = "http://tm.65960.com/templates/wed/mask-lightapp"
 ...
 */
 	...
 	page_position	: function(e,moveP,$self){ 		
        ...
        if(car2._touchDeltaY<=0){
        	if($self.next('.m-page').length == 0){
 			...
 			if($self.prev('.m-page').length == 0) return;
 			car2._pageNext = car2._pageNow-1;
 			next = car2._page.eq(car2._pageNext)[0];
 		}
 		...
 		function init_next(node){
	        ...
	        if(car2._movePosition=='up'){
 				...
 				if(s>0) l = car2._windowHeight+s;
 				else l = car2._windowHeight;
 				...
 				node[0].style[car2._prefixStyle('transform')] = 'translate(0,-'+Math.max(car2._windowHeight,$(node[0]).height())+'px)'+_translateZ;
 				$(node[0]).attr('data-translate',-Math.max(car2._windowHeight,$(node[0]).height()));
 	...
 	page_translate	: function(node){
		...
		scale = 1 - Math.abs(y*0.2/car2._windowHeight);
		y_2 = y_2/5;
 	...
 	page_success	: function(){
 		...
 		// 当前页面变小的移动
 		var y = car2._touchDeltaY > 0 ? car2._windowHeight/5 : -car2._windowHeight/5;
 		var scale = 0.8;
 	...
 	page_fial	: function(){
		...
		if(!car2._pageNext&&car2._pageNext!=0) {
 		...
 		if(car2._movePosition=='up'){
 			car2._page.eq(car2._pageNext)[0].style[car2._prefixStyle('transform')] = 'translate(0,'+car2._windowHeight+'px)'+_translateZ;
 		}else{
 			car2._page.eq(car2._pageNext)[0].style[car2._prefixStyle('transform')] = 'translate(0,-'+car2._windowHeight+'px)'+_translateZ;
 		}
 ...
 */
	...
	addMap	: function (detal,latitude,longitude,fnOpen,fnClose){
		...
		//默认值设定
		var a = {sign_name:'',contact_tel:'',address:'成都市'};
	...
	input_form : function(){
		$('body').on('click','.book-bd .bd-form .btn',function(){
			...
			$('.book-bg').on('click',function(e){
				...
				if(node.parents('.book-form').length>=1) return;
