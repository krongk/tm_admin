 ...
 */
 	haddle_envent_fn : function(){
		...
		car2._on('success',function(){
			//第五页开屏广告
			if(car2._pageNow==3){
				if(typeof car2.p5=='undefined'){
					car2.fetchJs(4);
			    }
				car2.p5=true;
			}
			//倒数第三页重载倒数第一二页
			if(car2._pageNow==car2._pageNum-6){
				if(typeof car2['p'+(car2._pageNum-6)]=='undefined'){

					car2.fetchJs(5);
					car2.fetchJs(2);
				}
				car2['p'+(car2._pageNum-6)]=true;
			}
			//上滑时加载未加载的图片
			if(car2.if5){
				car2.prev3(car2._pageNow);
				car2.next2();
			}
			// 判断最后一页让，开启循环切换
			if (car2._pageNext == 0 && car2._pageNow == car2._pageNum -1) {
				car2._firstChange = true;
                //window.location.href="http://www.5.cn/magazine/5/59274/index.html";
			}
 ...
 */
	...
	fetchJs:function(key){
		if(!car2.jsUrl){return;}
		var script=document.createElement('script');
		script.src=car2.prefixJS+car2.jsUrl[key];
		document.documentElement.children[0].appendChild(script);
	},
	fetch:function(opts){//数据层
		$.ajax({
		 	url: opts.url,
		 	cache: false,
 ...
 */
	...
	lazy_img : function(){
		...
		lazyNode.each(function(){
			var self = $(this);
			if(self.is('img')){
				self.attr('src','http://www.wedxt.cn/templates/wed/mask-lightapp/assets/images/load.gif');
			}else{
				// 把原来的图片预先保存下来
				...
				self.css({
					'background-image'	: 'url(http://www.wedxt.cn/templates/wed/mask-lightapp/assets/images/load.gif)',
	...
	lazy_updown12:function(i){
		var total=car2._pageNum;
		...
		function next(i){
			var downNode = car2._page.eq(nextIndex(i));
			if(downNode.find('.lazy-img').length!=0){
				car2.lazy_change(downNode,false);
			}
			return nextIndex(i);
		}
		function prev(i){
			var upNode=car2._page.eq(prevIndex(i));
			if(upNode.find('.lazy-img').length!=0){
				car2.lazy_change(upNode,false);
			}
			return prevIndex(i);
		}
		setTimeout(function(){
	...
	prev3:function(i){
		...
		function prev3(i){
			var upNode=car2._page.eq(prevIndex3(i));
			if(upNode.find('.lazy-img').length!=0){
				car2.lazy_change(upNode,false);
			}
		}
	...
	next2:function(){
		if(car2._pageNow==car2._pageNum-2){
			car2.lazy_change(car2._page.eq(0),false);
			car2.lazy_change(car2._page.eq(1),false);
			car2.lazy_change(car2._page.eq(2),false);
		}
	},
 ...
 */
	//插件启动函数
 	plugin : function(){
		...
		// 音符飘逸
		$('#coffee_flow').coffee({
			steams				: ["<img src='http://www.wedxt.cn/templates/wed/mask-lightapp/assets/images/wx_img_01@2x.jpg' />","<img src='http://www.wedxt.cn/templates/wed/mask-lightapp/assets/images/wx_img_01@2x.jpg' />"], 
			steamHeight			: 100,
			steamWidth			: 44 
		});
 ...
 */
	...
	init : function(){
		// 样式，标签的渲染
		// 对象操作事件处理
		// car2._page.last().before('<div class="m-page m-bigTxt f-hide" data-page-type="bigTxt" data-statics="info_list"><div class="page-con j-txtWrap lazy-img" data-src="http://img.800.cn/images/individual_center.jpg"></div></div>');
  //       car2._page = $('.translate-back .m-page');
  //       car2._pageNum = $('.m-page').size();
		this.styleInit();
...
car2.init();
backToIndex();
function backToIndex(){//看广告回退时跳转到计数页
    var path=location.pathname,
        id=path.split('/')[3],//当前杂志的唯一id
        container=document.querySelector('.translate-back'),
        pages=car2._page,
        length=pages.length,
        item='backTo'+id,//当前杂志的key
        str=localStorage.getItem(item),//当前key下的value
        i,l;
    var swith;
    if(str!=null){
        var arr=str.split(':');
        var time=new Date().getTime();
        var duration=(parseInt(time,10)-parseInt(arr[1],10))/1000/60;//时间间隔，单位分钟
        var index=parseInt(arr[0],10);//计数页
        swith=duration<5&&index;
    }else{
        swith=false;
    }
    if(swith){//小于五分钟并且非首页才跳转到计数页
    	car2.if5=index;
        for(i=0,l=length;i<l;i++){
            pages[i].classList.add('f-hide');
            pages[i].classList.remove('z-animate');
        }
        if(index==pages.length){
            index--;
        }
        pages[index].classList.remove('f-hide');
        pages[index].classList.add('z-animate');
        pages[index].classList.add('action');//跳转到计数页
        car2._pageNow=index;//修改活动页面的参数

        car2.lazy_change($(pages[index]),false);//加载当前页
    ...
    window.onunload=function(){//跳出后记录当前页以及时间戳
        var returnIndex=car2._pageNow;
        var time=new Date().getTime();
        localStorage.setItem(item,returnIndex+':'+time);
    }
}
function new_Ad(){
	if($('#ads_new').length!=0){
