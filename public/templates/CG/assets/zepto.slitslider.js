...
SlitSlider.prototype = {
  next:function(){
    ...
    if(itemIndex >= this.itemsNum-1 && !this.options.loop)
      return;
    if(itemIndex === this.itemsNum-1 && this.options.loop){
      this.items.eq(0).css('display','block').removeClass('selected');
      this.reset();
    }
    
    var aniItem = this.items.eq(itemIndex);
    this.items.eq(itemIndex+1).css('display','block').addClass('selected');
    this.items.eq(itemIndex+1).siblings().removeClass('selected');
    this.donext(aniItem);
  ...
  prev:function(){
    ...
    aniItem.css('display','block').addClass('selected');
    aniItem.siblings().removeClass('selected');
    this.doprev(aniItem);
  },
